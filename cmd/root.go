...
func initCli() {
	...
	rootCmd.Flags().StringVar(&ciConfigFile, "ci-config", ".docker-inspect-ci", "If CI=true in the environment, use the given yaml to drive validation rules.")
...
func initConfig() {
	...
	viper.SetDefault("log.path", "./docker-inspect.log")
	...
	viper.SetEnvPrefix("DOCKER-INSPECT")
...
func initLogging() {
	...
	log.SetLevel(level)
	log.Debug("Starting Docker-inspect...")
...
// and in $HOME/.config/docker-inspect/ directory
// defaults to $HOME/.docker-inspect.yaml
func getDefaultCfgFile() string {
	...
	return path.Join(home, ".docker-inspect.yaml")
}

// findInPath returns first "*.yaml" file in path's subdirectory "docker-inspect"
// if not found returns empty string
func findInPath(pathTo string) string {
	directory := path.Join(pathTo, "docker-inspect")
